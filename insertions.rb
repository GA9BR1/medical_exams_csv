  ...
  private
    ...
    def self.insert_patient_if_not_exists(object, db)
      ...
      if patient_not_exists
        patient_id = db.exec_params('INSERT INTO patients (name, cpf, email, birthday, address, city, state) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT(cpf) DO NOTHING',
    ...
    def self.insert_doctor_if_not_exists(object, db)
      ...
      if doctor_not_exists
        db.exec_params('INSERT INTO doctors (name, crm, crm_state, email) VALUES ($1, $2, $3, $4) ON CONFLICT(crm) DO NOTHING',
    ...
    def self.insert_test_type_if_not_exists(object, db)
      ...
      if test_type_not_exists
        db.exec_params('INSERT INTO test_types (name, limits) VALUES ($1, $2) ON CONFLICT(name) DO NOTHING',
    ...
    def self.insert_test_and_test_items_if_not_exist(object, db)
      ...
      test_type_id = db.exec_params('SELECT id FROM test_types WHERE name = $1', [object['tipo exame']])[0]['id'].to_i
        if test_not_exists
          ...
          db.exec_params('INSERT INTO tests (token, patient_id, doctor_id, date) VALUES ($1, $2, $3, $4) ON CONFLICT(token) DO NOTHING',
          ...
          db.exec_params('INSERT INTO test_items (result, test_type_id, test_id) VALUES ($1, $2, $3) ON CONFLICT(test_id, test_type_id) DO NOTHING',

...
describe 'tests the query method' do
  def app
    Sinatra::Application
  end
  ...
  it 'single one with token successfully' do
    db = PG.connect(host: 'postgres-server-test', user: 'postgres')
    json = QueryAndFormat.get_single_test('NIG0TP')
    json2 = QueryAndFormat.get_single_test('T9O6AI')
    db.close
  ...
  it 'query all sucessfully' do
    db = PG.connect(host: 'postgres-server-test', user: 'postgres')
    json = QueryAndFormat.get_all_tests
    db.close
